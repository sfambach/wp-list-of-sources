...
function wpls_sort_sources($a, $b) {
    // KORRIGIERT: Alle Klammern und Abfragen sauber geschlossen
...
function wpls_render_sources_table( $attributes, $content ) {
    ...
    foreach ($links as $link) {
        ...
        // GEFIXT: Fehlendes $ bei $title ergänzt
        if(!empty($title)){
            $final_title = $title; 
        } else if(!empty($text)){
            $final_title = $text;
        ...
        // GEFIXT: Das Auskommentieren/Überschreiben entfernt, damit der Titel nicht verloren geht!
